...
function showUnavailableImage(target,size){
...
$(function() {
	...
	$("img.lazyImage").lazyload({
		placeholder: "/img/loader_avatar.gif",
		threshold: 300,
		failurelimit: 3,
		effect: "fadeIn"
	});
	...
	if($.cookie('u') != null){
		...
		if(cookie && cookie.split('|').length > 0){
			...
			$('#u_entrar_topo').hide();
			$('#loginTop').addClass('logged').html('<h3 class="welcome">Bem vindo<span class="baseColor">.</span>ai</h3><br />' + user.name + ' <br /> <a href="/u/conta">Minha Conta</a> | <a href="/u/sair">Sair</a>');
		}
	}
